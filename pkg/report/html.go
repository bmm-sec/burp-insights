...
// GenerateHTML writes a report to the provided writer using the report template.
func GenerateHTML(w io.Writer, report *Data, opts Options) error {
	if strings.TrimSpace(opts.Title) == "" {
		opts.Title = "Burp Project Report"
	}

	view := buildReportView(report, opts)

	tmpl, err := loadReportTemplate(opts.TemplatePath)
	if err != nil {
		return err
	}
...
func buildReportView(report *Data, opts Options) reportView {
	now := time.Now()
...
func buildSummaryCards(report *Data, opts Options, stats Stats) []reportCardView {
...
func buildIssuesSection(issues []burp.ScannerIssueMeta, opts Options) *issuesSectionView {
	total := len(issues)
...
func buildIssueRowView(index int, issue burp.ScannerIssueMeta, opts Options, typeID string) issueRowView {
...
func buildTasksSection(tasks []burp.UITask, opts Options) *tasksSectionView {
	total := len(tasks)
...
func buildRepeaterSection(tabs []string, opts Options) *repeaterSectionView {
	total := len(tabs)
...
func buildHistorySection(entries []burp.HTTPEntry, opts Options) *historySectionView {
	total := len(entries)
